...
class Agent:
    ...
    async def _send_message(
        ...
        *,
        ...
        tool_iterations = 0
        previous_tool_calls: List[str] = []  # Track tool calls to detect loops
        ...
        while tool_iterations < self.config.max_tool_iterations:
            ...
            # If we have substantial content (not just empty string), prefer it over tool calls
            # This helps break loops where LLM keeps making tool calls instead of answering
            has_substantial_content = response.content and response.content.strip() and len(response.content.strip()) > 10
            
            # Aggressive loop breaking: if we've done many tool iterations and still only getting tool calls
            # (no content), break to prevent infinite loops
            # This check happens AFTER we get the response but BEFORE we execute tools
            if tool_iterations >= 5 and response.is_tool_call() and not has_substantial_content:
                logger.warning(f"Breaking tool loop after {tool_iterations} iterations with only tool calls and no content. "
                            f"This suggests the LLM may not be recognizing tool responses properly.")
                # Force break - don't execute these tools, just exit
                # The agent will show the tool limit warning
                break
            
            # Check for duplicate tool calls (infinite loop detection)
            if response.tool_calls:
                # Create signatures safely - convert arguments to JSON-serializable format
                def safe_serialize_args(args: dict) -> str:
                    """Safely serialize arguments to string, handling non-serializable objects."""
                    try:
                        # Try direct JSON serialization first
                        return json.dumps(args, sort_keys=True, default=str)
                    except (TypeError, ValueError):
                        # If that fails, convert to a simple string representation
                        ...
                        for key, value in args.items():
                            try:
                                ...
                                # Not serializable, use string representation
                                serializable_args[key] = str(value)
                        return json.dumps(serializable_args, sort_keys=True)
                
                current_tool_call_signatures = [
                    f"{tc.name}:{safe_serialize_args(tc.arguments)}" 
                    for tc in response.tool_calls
                ]
                if current_tool_call_signatures == previous_tool_calls:
                    # Same tool calls as before - likely stuck in a loop
                    logger.warning(f"Detected duplicate tool calls at iteration {tool_iterations}, breaking loop")
                    # If we have content, use it; otherwise break anyway
                    if has_substantial_content:
                        break
                    else:
                        # Force break to avoid infinite loop
                        break
                previous_tool_calls = current_tool_call_signatures

            # Handle tool calls
            if response.is_tool_call() and not has_substantial_content:
                tool_iterations += 1
